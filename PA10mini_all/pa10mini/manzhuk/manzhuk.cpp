...
con1t5=1.5e0;conm3=1e-3;conm12=1e-12;conmst=1e-32/*-200*/;
...
fprintf(f01," conm14 = %15.8e \n\n",conm14);
      ...
      nip=nlkk+n;
      nip1=nip+n;
      for(i=1;i<=20+2*n+m;i++) ip[i]=0;
      for(i=1;i<=2*n*m+4*n*n+15*n+18*m+4;i++) ar[i]=con0;
      h=hmn;
      if(ih==0&&n>m)
       {  for(i=m+1;i<=n;i++) z[i]=con0;
      ...
      ipkp=0;
      ipm=0;
      for(i=1;i<=n;i++) ar[i+nzn]=z[i];
      for(i=1;i<=m;i++) ar[i+ndx1]=px[i];
d5:        ;;
      for(i=1;i<=m;i++)
       { px1[i]=px[i];
        px[i]=ar[i+ndx1];}
...
d10:       ;
     ...
     for(i=1;i<=m;i++)
       ...
       ra0=con1;
       ih1=0;
       if (fabs(px[i])>conm14/*conmst*/)
        { ra1=fabs(px1[i]/px[i]);
         ra2=fabs((px1[i]-px[i])/px[i]);
         for(ii=1;ii<=10;ii++)
          { ra0=ra0*con2;
           if(fabs(ra1-ra0)<cont1) ih1=1;}
         if(ih1==0)
          {
          if(ra2>ee) ee=ra2;
          };
...
d30:       ;;
         ...
         if(tt<ht) goto d40;
         iprh=2;
d40:       if(iprh==0) t=tn+h;
     if(iprh==2) t=tn+tt;
     if((t+h/2e0)<(*tkv)) goto d60;
     if(fabs(tn-(*tkv))<ht) goto d50;
     h=(*tkv)-tn;
     t=tn+h;
d50:	 ipkv=1;
d60:       ;;
     if((t+h/2e0)<tk) goto d80;
     if(fabs(tn-tk)<ht) goto d70;
     h=tk-tn;
     t=tn+h;
d70:       ipk=1;
d80:	 prwar=0;
     ...
     kolil=0;
     ip[1]=0;
     for(i=1;i<=n;i++) ar[i+nmz1]=fabs(ar[i+nzn]);
     for(i=1;i<=m;i++)
       {ar[i+ndarx]=con0;
            ar[i+ndar1]=con0;
        ar[i+ndar]=con0;
     ...
     for(i=1;i<=m;i++)
       {
        ...
        if(ra1<ra2) ra1=ra2;
        ra2=0e0;
            if(ra1>conmst) ra2=fabs(r11/ra1);
        if(ra2<conm14) lkt=0;
        if(rxnm1<0e0) sn1=1;
        else sn1=0;
        if(rxnm2<0e0) sn2=1;
        ...
        if(ra1<ra2) ra1=ra2;
        ra2=0e0;
        if(ra1>conmst) ra2=fabs(r22/ra1);
        if(ra2<conm14) lkt1=0;
        if(rxnm2<0e0) sn1=1;
        else sn1=0;
        if(rxnm3<0e0) sn2=1;
        else sn2=0;
        ...
        if(ra1<ra2) ra1=ra2;
        ra2=0e0;
        if(ra1>conmst) ra2=fabs(r33/ra1);
        if(ra2<conm14) lkt2=0;
        ip[i+nlkk]=lkt+lkt1+lkt2;
        if(ip[i+nlkk]==3) ipkor=1;
     };
     if(ipm==1) goto d170;
     if(ipkor==0) goto d150;
     ip[12]=ip[12]+1;
         ipkp=2;
     for(i=1;i<=m;i++)
        {ar[i+ndx1]=px[i];
...
d150:      r0=h;
         ...
         ar[i+nxnm2]=ar[i+ndx1];
          if(ip[i+nlkk]==3)
          {
           ...
           ar[i+nxnm2]=px1[i];
            ...
            px[i]=px1[i];
        ar[i+nxn]=px1[i];
        if(ar[i+nmz]<fabs(ar[i+nzn])) ar[i+nmz]=fabs(ar[i+nzn]);
    };
     ip[3]=ip[3]+1;
     ip[10]=ip[10]+1;
     out(z,px,n,m,t,t0,tk,h,tkv,ncon,ip);
     ...
     if(em4>epd16) ih3=0;
     if(ipkv!=0) goto d160;
         if(em4>epd16) goto d160;
         if(h==hpred&&em4>epd32) ih=ih+1;
         if(h==hpred&&ih<3&&em4>epd32) goto d160;
         if(em4>epd32) ih=0;
         if(ih==0&&em4>epd32) r0=h*con1t1;
     if(em4>epd32) goto d160;
         if(h==hpred&&em4>epd64) ih1=ih1+1;
         if(h==hpred&&ih1<3&&em4>epd64) goto d160;
         if(em4>epd64) ih1=0;
         if(ih1==0&&em4>epd64) r0=h*con1t2;
     if(em4>epd64) goto d160;
         if(h==hpred&&em4>epd128) ih2=ih2+1;
         if(h==hpred&&ih2<3&&em4>epd128) goto d160;
         if(em4>epd128) ih2=0;
         if(ih2==0&&em4>epd128) r0=h*con1t5;
     if(em4>epd128) goto d160;
     if(em4<conm7) ih3=3;
     if(h==hpred) ih3=ih3+1;
     if(h==hpred&&ih3<3) goto d160;
         ih3=0;
     r0=h*con2;
...
d180:      ;
     ...
     ipm=1;
     for(i=1;i<=m;i++)
       { if(ip[i+nlkk]!=3) goto d190;
            ar[i+ndx1]=px[i];
d190:      ;};
     for(i=1;i<=n;i++) z[i]=ar[i+ndz1];
     for(i=1;i<=m;i++) px[i]=ar[i+ndx1];
     ipkp=0;
d200:      r0=h;
     for(i=1;i<=n;i++)
       { z1[i]=z[i];
     ...
     out(z,px,n,m,t,t0,tk,h,tkv,ncon,ip);
         ...
         if(etr>epd8) ih2=0;
     if(ipkv!=0) goto d210;
         if(etr>epd8) goto d210;
         if(h==hpred&&etr>epd16) ih=ih+1;
         if(h==hpred&&ih<3&&etr>epd16) goto d210;
         if(etr>epd16) ih=0;
         if(ih==0&&etr>epd16) r0=h*con1t1;
         if(etr>epd16) goto d210;
         if(h==hpred&&etr>epd32) ih1=ih1+1;
         if(h==hpred&&ih1<3&&etr>epd32) goto d210;
         if(etr>epd32) ih1=0;
         if(ih1==0&&etr>epd32) r0=h*con1t3;
         if(etr>epd32) goto d210;
     if(etr<conm7) ih2=3;
         if(h==hpred) ih2=ih2+1;
         if(h==hpred&&ih2<3) goto d210;
         ih2=0;
     r0=h*con2;
d210:	 if(etr<epd8) goto d230;
         ipm=2;
     goto d230;
d220:      r0=h;
     for(i=1;i<=n;i++) ar[i+nzn]=z[i];
     ...
     dp=fdz;
     if(ipkv!=0) goto d230;
     if(ee>epd4) goto d230;
         r0=h*con1t2;
         if(ee>epd8) goto d230;
         r0=h*con1t5;
         if(ee>epd16) goto d230;
     r0=h*con2;
d230:      ;
     hpred=h;
     if(nm==1) ipm=0;
     ...
     goto d30;
d240:     ;
     for(i=1;i<=2*(n*n);i++) ar[i+na12]=con0;
     for(i=1;i<=m;i++)
       {r11=(a11*ar[i+ndar]+a12*ar[i+ndar1]);
        ...
        ar[i+nbxp1]=-(ah*r22+ar[i+ndarx]);
         ...
         tv=tn;
         if(iprh==0) tv=tn+h/con2;
         if(iprh==2) tv=tn+tt/con2;
     for(i=1;i<=n;i++)
       {f[i]=conmd;
        if(fabs(z[i])>conmst) f[i]=conmd*fabs(z[i]);
       };
...
/*fprintf(f01,"ia=%d,ar[ia+na11]=%15.8e\n",ia,ar[ia+na11]);	       */
     ...
     if(iprp==1)
     {
      ...
      for(i=n;i>=1;i--)
        {
         ra0=conmd;
         if(fabs(z[i])>conmst) ra0=conmd*fabs(z[i]);
         z[i]=z[i]+ra0;
         (*nbad)=0;
         ncon=3;
         fct(z,px,f,rj1,rj2,n,m,tv,h,ncon,nbad,ip);
         ncon=1;
         if((*nbad)==1) goto d510;
         for(j=1;j<=n;j++) rj2[j*n+i]=(f[j]-ar[j+nzr1])/ra0;
         z[i]=ar[i+nzr];
       };
      for(i=1;i<=n;i++) f[i]=ar[i+nzr1];
     };
     for(j=1;j<=n;j++)
       {for(i=1;i<=n;i++)
...
/*fprintf(f01,"ia=%d,ar[ia+na11]=%15.8e\n",ia,ar[ia+na11]);	       */
     ...
     for(i=1;i<=n;i++) ar[i+nb]=-f[i];
     for(i=1;i<=n;i++)
       {f[i]=conmd;
        if(fabs(z1[i])>conmst) f[i]=conmd*fabs(z1[i]);
        };
     (*nbad)=0;
     nsm4=2;
     fct(z1,px1,f,rj1,rj2,n,m,t,h,ncon,nbad,ip);
     if((*nbad)!=0) goto d510;
d260:	 ia=1;
     for(j=1;j<=m;j++)
        {for(i=1;i<=n;i++)
...
/*fprintf(f01,"ia=%d,ar[ia+na22]=%15.8e\n",ia,ar[ia+na22]);	       */
     ...
     if(iprp==1)
      ...
      for(i=n;i>=1;i--)
        { ra0=conmd;
         if(fabs(z1[i])>conmst) ra0=conmd*fabs(z1[i]);
         z1[i]=z1[i]+ra0;
         (*nbad)=0;
         ncon=3;
         fct(z1,px1,f,rj1,rj2,n,m,t,h,ncon,nbad,ip);
...
/*fprintf(f01,"ia=%d,ar[ia+na22]=%15.8e\n",ia,ar[ia+na22]);	       */
           ia=ia+1;};};
     for(i=1;i<=n;i++) ar[i+nb1]=-f[i];
         if(ip[1]>1) goto d270;
     for(i=1;i<=2*n;i++) if(fabs(ar[i+nb])>fnm) fnm=fabs(ar[i+nb]);
     for(i=1;i<=2*m;i++) if(fabs(ar[i+nbxp])>fnm) fnm=fabs(ar[i+nbxp]);
d270:      fcon=con0;
     fconp=con0;
     for(i=1;i<=2*n;i++)
     {
        if(fabs(ar[i+nb])>fcon) fcon=fabs(ar[i+nb]);
        if(fabs(ar[i+nb])>fconp) fconp=fabs(ar[i+nb]);
     };
     for(i=1;i<=2*m;i++) if(fabs(ar[i+nbxp])>fcon) fcon=fabs(ar[i+nbxp]);
     if(ip[1]==0) goto d280;
     if(fnm<conmst) goto d380;
     radz=con1;
     for(i=1;i<=2*n;i++)
     {
        ra1=con1;
        ...
        ra3=fabs(ar[i+ndz]);
        ra4=fabs(ar[i+ndzs]);
        if(ra3>conmst) ra1=ra4/ra3;
        if(ra4>conmst) ra2=ra3/ra4;
        if(ra2>ra1) ra1=ra2;
        if(radz<ra1) radz=ra1;
        };
     rafc=con0;
     if(fnm>conmst) rafc=fcon/fnm;
     if(rafc<conm7&&ip[1]==1) kolf=1;
     rafp=con1;
     if(fconp>conmst) rafp=fp/fconp;
     if(fp<conmst) rafp=con1;
     radzp=con1;
     if(fdz>conmst) radzp=dp/fdz;
d280:      ;;
 //printf("fcon= %15.8e fnm= %15.8e fdz=%15.8e h=%15.8e\n",fcon,fnm,fdz,h);
...
/*
    ...
    for(i=1;i<=n;i++)fprintf(f01,"%15.8e ",z1[i]);
             fprintf(f01,"\n");
    for(i=1;i<=m;i++)fprintf(f01,"%15.8e ",px1[i]);
             fprintf(f01,"\n");
*/
     if(ip[1]<2) goto d290;
...
d300:         if(ip[1]<3) goto d310;
        if(ar[i+ndzs]==con0)  ar[i+ndz]=con0;
...
d320:         if(ip[1]<3) goto d330;
        if(ar[n+i+ndzs]==con0)  ar[i+ndz1]=con0;
d330:     ;};
     for(i=1;i<=m;i++)
     {
            ra2=fabs(px[i]);
...
d340:         if(ip[1]<3) goto d350;
        if(ar[i+ndxs]==con0)  ar[i+ndx]=con0;
d350:         ra2=fabs(px1[i]);
            ra3=fabs(ar[i+ndx1]);
            if(ra2>conmst) ra3=ra3/ra2;
...
d360:         if(ip[1]<3) goto d370;
        if(ar[n+i+ndxs]==con0)  ar[i+ndx1]=con0;
d370:      ;   };
     for(i=1;i<=n;i++)
     {
            z[i]=z[i]+ar[i+ndz];
        z1[i]=z1[i]+ar[i+ndz1];
            if(ar[i+nmz1]<fabs(z[i])) ar[i+nmz1]=fabs(z[i]);
        if(ar[i+nmz1]<fabs(z1[i])) ar[i+nmz1]=fabs(z1[i]);
     };
     for(i=1;i<=m;i++)
     {
            px[i]=px[i]+ar[i+ndx];
        px1[i]=px1[i]+ar[i+ndx1];
     ...
     for(i=1;i<=n;i++)
     {
            if(ar[i+nmz1]>conmst) ar[i+ndz1]=ar[i+ndz1]/ar[i+nmz1];
        if(ar[i+nmz1]>conmst) ar[i+ndz]=ar[i+ndz]/ar[i+nmz1];
        if(fabs(ar[i+ndz])<conm14) ar[i+ndz]=con0;
        if(fabs(ar[i+ndz1])<conm14) ar[i+ndz1]=con0;
            if(fabs(ar[i+ndz1])>fdz) fdz=fabs(ar[i+ndz1]);
        if(fabs(ar[i+ndz])>fdz) fdz=fabs(ar[i+ndz]);
     };
     goto d240;
d380:      ;;
     ip[2]=ip[2]+ip[1];
     ip[9]=ip[9]+ip[1];
         h1=h;
     for(i=1;i<=m;i++) ar[i+nzt]=z1[i];
     goto d560;
d390:     ;
     if(ipm==1)
     {
      for(i=1;i<=m;i++) ar[i+nbxp]=-(px[i]+ah*ar[i+ndar]+ar[i+nxn]);
      }
     else
     {
      for(i=1;i<=m;i++) ar[i+nbxp]=-(px[i]+ah*ar[i+ndar]);
      };
     for(i=1;i<=n;i++)
     {
        f[i]=conmd;
...
//fprintf(f01,"rj2: ia=%d,ar[ia+na11]=%15.8e\n",ia,ar[ia+na11]);
     ...
     if(ip[1]>1) goto d410;
     for(i=1;i<=n;i++)
        if(fabs(ar[i+nb])>fnm) fnm=fabs(ar[i+nb]);
     for(i=1;i<=m;i++)
        if(fabs(ar[i+nbxp])>fnm) fnm=fabs(ar[i+nbxp]);
d410:      fcon=con0;
     fconp=con0;
     for(i=1;i<=n;i++)
     {
     ...
     for(i=1;i<=m;i++)
        if(fabs(ar[i+nbxp])>fcon) fcon=fabs(ar[i+nbxp]);
     if(fnm<conmst) goto d480;
     if(ip[1]==0) goto d420;
     radz=con1;
     for(i=1;i<=n;i++)
     {
...
/*
     printf("ip[1]= %d fnm= %15.8e  fcon= %15.8e fdz= %15.8e h= %15.8e \n ", ip[1], fnm, fcon, fdz, h);
...
d540:      ip[16]=ip[16]+1;
 d550:      ip[6]=ip[6]+1;
     printf("02 ip[1]= %d *ier= %d ipm= %d\n ", ip[1], *ier, ipm);
     if(ipm==2)
     {
      if((*nbad)==2&&h<=2e0*hmn&&nsm4==1) goto d250;
      if((*nbad)==2&&h<=2e0*hmn&&nsm4==2) goto d260;}
     else
     {
      if((*nbad)==2&&h<=2e0*hmn) goto d400;};
     if(h<=hmn) goto d580;
     if(ipkp==1) ipm=1;
     if(ipkp==2) ipm=2;
     goto d570;
d560:    ;
     ...
     ar[2]=etr;
         em4=em4*con64;
     ar[3]=em4;
     if(prwar==1&&eps>100e0) goto d565;
     if(prwar==1&&ar[1]>conm7) goto d500;
d565:     ;
     if(prwar==1) ip[11]=ip[11]+1;
     if(prwar==1) printf(" warning! fdz>0.01: t=%15.8e ,h=%15.8e ,ar[1]=%15.8e  \n",t,h,ar[1]);
     if(ipm==0)
     {
       if(ee>epsp) goto d490;
       goto d220;};
     if(ipm==1)
     {
       if(etr>epsp) goto d490;
       goto d120;};
     if(ipm==2)
     {
       if(em4>epsp) goto d490;
       goto d120;};
d570:      ;
     h=h/con2;
     for(i=1;i<=n;i++)
     ...
     ip[4]=ip[4]+1;
     ip[2]=ip[2]+ip[1];
     if(ipm==2) ip[9]=ip[9]+ip[1];
     if(ipm==1) ip[7]=ip[7]+ip[1];
     ipk=0;
     ipkv=0;
     ipkp=0;
     goto d30;
d580:      if((*ier)==8) goto d590;
d590:      ;
     ipk=0;
     ipkv=1;
     t0=t;
